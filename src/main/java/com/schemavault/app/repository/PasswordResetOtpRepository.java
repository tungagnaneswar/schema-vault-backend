...
public interface PasswordResetOtpRepository extends JpaRepository<PasswordResetOtp, Long> {
   ...
   *
   * <p>
   * Uses a pessimistic write lock ({@code SELECT ... FOR UPDATE}) so that
   * concurrent
   * requests for the same OTP are serialized at the database level. The second
   * request
   * will block until the first commits, then re-read and find the record already
   * consumed
   * (attempt_count updated or is_used set), preventing double-verification.
   *
   * <p>
   * {@code jakarta.persistence.lock.timeout = 3000} sets a 3-second timeout so
   * the
   * request fails fast rather than blocking indefinitely if another transaction
   * holds the lock.
   */
   ...
   * The partial index {@code idx_otp_expires_at} keeps this efficient on large
   * datasets.
   */
  @Modifying
   ...
   * Called by the hourly cleanup job to prevent stale used-records from
   * accumulating.
   */
  @Modifying
